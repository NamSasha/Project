...
def count_maxmin():
    global a
    global b
    a = int(input("Input Over spec: "))
    b = int(input("Input Under Spec: "))
    count_max = 0
    count_min = 0
    for i in range(len(df["H-Judge"])):
        ...
        if df["H-Judge"][i]<=b-1 and df["H-Judge"][i]!=0:
            count_min +=1  
...
ax = plt.gca()
above_spec = np.full(shape=len(df['Date_Time']),fill_value=a)
minimum_spec = np.full(shape=len(df['Date_Time']),fill_value=b)
